...
if __name__ == '__main__': # <- Executable 
    ...
    while not camera_found:
        ...
        pipeline = 'nvarguscamerasrc sensor-id=' + camera_index + ' ! video/x-raw(memory:NVMM), width=(int)1920, height=(int)1080, format=(string)NV12, framerate=(fraction)15/1 ! nvvidconv ! video/x-raw, format=(string)BGRx ! videoconvert ! video/x-raw, format=(string)BGR ! appsink'
